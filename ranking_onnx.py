...
def main():
    configs = get_configs()
    logger.critical(f'Running on GPU: {configs.gpu}')
...
def score_model(configs, score_loader):
    ...
    dummy_input = torch.randn(1, 3, 224, 224).cuda()  # For most models
    if configs.model == 'inception_v3':
        dummy_input = torch.randn(1, 3, 299, 299).cuda()  # For inception_v3

    input_names = ["input"]
    output_names = ["output"]
    torch.onnx.export(model, dummy_input, f"onnx/{configs.model}.onnx", verbose=True, input_names=input_names, output_names=output_names)

    # # Compare the outputs of PyTorch and ONNX models using find_mismatch
    # model.eval()
    # with torch.no_grad():
    #     pytorch_output = model(dummy_input).cpu().numpy()
    
    # onnx_file = f"onnx/{configs.model}.onnx"
    
    # Compare the outputs of PyTorch and ONNX models using find_mismatch
    # dummy_input_cuda = dummy_input.cuda()
    graph_info = find_mismatch(model, (dummy_input,))  # You may need to adjust the opset_version

    if graph_info is not None:
        logger.info(f"Mismatch found between {configs.model} PyTorch and ONNX outputs:")
        logger.info(graph_info)
    else:
        logger.info(f"No mismatch found between {configs.model} PyTorch and ONNX outputs.")



    # different models have different linear projection names
